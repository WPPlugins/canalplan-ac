 ...
 *
 *
 * Copyright 2011 - 2017 Steve Atty (email : posty@tty.org.uk)
 *
...
define('CANALPLAN_CODE_RELEASE','3.45 r00');
define('CANALPLAN_DB_VERSION',2);
...
function update_canalplan($post_id,$silent,$sleep,$api) {
	...
	$blog_url=get_bloginfo('url');	
		...
		if ($silent =='N') echo "<br />Processing Post <i>".$bulkpost->post_title."</i><br />";
		...
		if (preg_match_all('/' . preg_quote('[[CP:') . '(.*?)' . preg_quote(']]') .'/',$postcontent,$matches)) {
			...
			foreach ($places_array as $place) {
				...
				if ($silent =='N') 	echo "&nbsp;&nbsp;&nbsp;Found link to <i>".$placeinfo[0]."</i>";
				...
				if ($silent =='N'){
					...
					if ($silent =='N') echo "&nbsp;&nbsp;&nbsp;<b>A problem occurred : ".$cp_bulk['status']." - ".$cp_bulk['detail']."</b><br />";
				}
			...
			if ($silent =='N')echo "&nbsp;&nbsp;&nbsp;No Canalplan Links Found<br />"; 
		}
}
...
//echo $row['lat']." ".$row['long']." ".addslashes($row['place_name'])."<br/>";
		if (count($row)> 2 && strlen($row['lat']) >0 && strlen($row['long']) >0 ) {
			...
			if ($do_plot==1) {
		//    echo $place." : ".$row['place_name'].' :'.$row['lat']." : ".$row['long']." : ".$lat_dif." : ".$long_dif." : ".abs($lat_dif)." : " .abs($long_dif)."<br/>";
...
//	echo $row['lat'].' - '.$row['long'].' - '.strlen($row['lat']).' - '.strlen($row['long']).' - '.$do_plot."<br/>";
...
{
//	preg_match_all('/^\{BLOGGEDROUTES|[a-zA-Z0-9]{1,100}}/',$content,$matches);
//	var_dump($matches);
...
function canal_activate() {
	...
	$sql='CREATE TABLE '. CANALPLAN_ALIASES. ' (
  ...
  PRIMARY KEY  (canalplan_id,place_name)
	...
	$sql= 'CREATE TABLE '.CANALPLAN_CODES.' (
  canalplan_id varchar(10) NOT NULL,
  place_name varchar(250) NOT NULL,
  ...
  SPATIAL INDEX (lat_lng_point)
	...
	$sql='CREATE TABLE '.CANALPLAN_FAVOURITES.' (
  ...
  KEY canalplan_idx (canalplan_id)
	...
	$sql='CREATE TABLE '.CANALPLAN_LINK.' (
  ...
  KEY waterway (waterway)
	...
	$sql='CREATE TABLE '.CANALPLAN_OPTIONS.' (
  blog_id bigint(20) NOT NULL DEFAULT 0,
  ...
  PRIMARY KEY  (blog_id,pref_code)
	...
	$sql='CREATE TABLE '.CANALPLAN_ROUTES.' (
  ...
  KEY status (status)
	...
	$sql='CREATE TABLE '.CANALPLAN_CANALS.' (
  ...
  KEY parent (parent)
	...
	$sql='CREATE TABLE '.CANALPLAN_POLYLINES.' (
  ...
  PRIMARY KEY  (id)
	...
	$sql='CREATE TABLE '.CANALPLAN_ROUTE_DAY.' (
  route_id bigint(10) NOT NULL DEFAULT 0,
...
//			_e("$errormsg<br />\n");
	...
	$sql="update ".CANALPLAN_OPTIONS." set pref_value=".CANALPLAN_DB_VERSION." where blog_id=-1 and pref_code='dbv'";
	$r = $wpdb->query("SELECT pref_value FROM  ".CANALPLAN_OPTIONS." where blog_id=-1 and pref_code='dbv'");
	if ($wpdb->num_rows==0) {
		$sql="insert into  ".CANALPLAN_OPTIONS." (pref_value,blog_id,pref_code) values ('".CANALPLAN_DB_VERSION."',-1,'dbv')";
	}
	$result = $wpdb->query( $sql );
}
...
function canal_update($dbv){
	...
	if ($dbv==0) {
		...
		$result = $wpdb->query( $sql );
	//	var_dump($result);
		if ($result === false) $errors[] = __('Failed to update ') . CANALPLAN_ROUTE_DAY ;
		if ($result === true) $updated = 1;
		
		$sql='ALTER TABLE '.CANALPLAN_ROUTES.' ADD `total_coords` longtext NULL';
		$result = $wpdb->query( $sql );
	//	var_dump($result);
		if ($result === false) $errors[] = __('Failed to update ') . CANALPLAN_ROUTES ;
		if ($result === true) $updated = 1;
	}
...
//		var_dump($schedule);
	if ($schedule !='daily' ) wp_schedule_event( time(), 'daily', 'canalplan_update_by_cron' );
...
function canalplan_update_cp_on_publish( $new_status, $old_status, $post ){
	global $blog_id,$wpdb;
	$post_id = $post->ID;
	if ($_POST['UpdateCanalplan']=='UpdateCanalplan' && $new_status=='publish') { 
		...
		if ($wpdb->num_rows>0) {
			...
			update_canalplan($post_id,'Y',10,$api);
		 }
	}
}
...
function canalplan_update_by_cron_function () {
	...
	if ($wpdb->num_rows>0) {
		...
		foreach ($r as $rw) {
			update_canalplan($rw['ID'],'Y',10,$api);
		}
	}
}
