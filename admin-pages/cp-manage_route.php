...
if ($cpsession!="0" && !isset($_POST['recalculate'])){
	$reimported=0;
...
//			var_dump($postkey);
	...
	//		$iskey=strpos($postkey,'current_post');
        //     echo $iskey;
	//		echo "<br >";
		if ($iskey === false) { } else {
		$alt_key=str_ireplace('current_post','alt_posts',$postkey);
		$day_key=str_ireplace('current_post','day_id',$postkey);
//	    echo $postkey." = ".$_POST[$postkey].' and '.$alt_key.' = '.$_POST[$alt_key]."<br />";
	    if ($_POST[$postkey] <> $_POST[$alt_key]) { 
			//echo "Change the Post ID <br />";
			$sql=$wpdb->prepare("update ".CANALPLAN_ROUTE_DAY." set post_id=%d where blog_id=%d and route_id=%d and day_id=%d",$_POST[$alt_key],$blog_id,$_POST["route_list"],$_POST[$day_key]);
			$res2 = $wpdb->query($sql);
			 }
	}
}
...
?>
<?php
$r =  $wpdb->prepare("SELECT distinct route_id,title,description,totalroute,duration,uom,status,routetag FROM ".CANALPLAN_ROUTES." where blog_id=%d and route_id=%d ORDER BY `start_date` ASC",$blog_id,$_POST["route_list"]);
...
$r = $wpdb->prepare("SELECT distinct day_id,route_date,start_id,end_id,distance,locks,flags,post_id FROM ".CANALPLAN_ROUTE_DAY." where blog_id=%d and route_id=%d and day_id > 0 ORDER BY `route_date` ASC",$blog_id,$_POST["route_list"]);
$r = $wpdb->get_results($r,ARRAY_A);
$stopplaces="";
$hidden_posts='';
foreach($r as $rw) {
...
//echo $posts_for_today;
	...
	if ($posts_for_today > 1) {
		...
		foreach ($alternative_posts as $alternative_post) {
				...
				if ($rw['post_id']==$alternative_post['postid']){echo ' selected="yes"';}
